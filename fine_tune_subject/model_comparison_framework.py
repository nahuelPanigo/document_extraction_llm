...
# Optional torch import for neural network
try:
    import torch
...
from constants import CSV_FOLDER, CSV_FORD_SUBJECTS, TXT_FOLDER, SUBJECT_MODEL_FOLDERS, SUBJECT_MODEL_RESULTS_FOLDER
import random
import os
import joblib
import pickle
...
class SVMStrategy(ModelStrategy):
    """SVM model strategy with configurable kernel"""
    ...
    def __init__(self, kernel='linear'):
        ...
        if kernel == 'linear':
            self.folder_key = "svm_linear"
        elif kernel == 'rbf':
            self.folder_key = "svm_rbf"
        else:
            self.folder_key = "svm"  # fallback
    
    def load_model(self):
        try:
            model_folder = SUBJECT_MODEL_FOLDERS[self.folder_key]
            self.clf = joblib.load(model_folder / 'svm_classifier.pkl')
            self.vectorizer = joblib.load(model_folder / 'svm_vectorizer.pkl')
            self.label_encoder = joblib.load(model_folder / 'svm_label_encoder.pkl')
            return True
        except FileNotFoundError:
            return False
    
    def predict(self, X_test_text):
        X_test_features = self.vectorizer.transform(X_test_text)
        y_pred_encoded = self.clf.predict(X_test_features)
    ...
    def get_model_files(self):
        model_folder = SUBJECT_MODEL_FOLDERS[self.folder_key]
        return [model_folder / 'svm_classifier.pkl', model_folder / 'svm_vectorizer.pkl', model_folder / 'svm_label_encoder.pkl']
...
class XGBoostStrategy(ModelStrategy):
    ...
    def load_model(self):
        try:
            model_folder = SUBJECT_MODEL_FOLDERS['xgboost']
            self.clf = joblib.load(model_folder / 'xgboost_classifier.pkl')
            self.vectorizer = joblib.load(model_folder / 'xgboost_vectorizer.pkl')
            self.label_encoder = joblib.load(model_folder / 'xgboost_label_encoder.pkl')
            return True
        except FileNotFoundError:
            return False
    ...
    def get_model_files(self):
        model_folder = SUBJECT_MODEL_FOLDERS['xgboost']
        return [model_folder / 'xgboost_classifier.pkl', model_folder / 'xgboost_vectorizer.pkl', model_folder / 'xgboost_label_encoder.pkl']
...
class RandomForestStrategy(ModelStrategy):
    ...
    def load_model(self):
        try:
            model_folder = SUBJECT_MODEL_FOLDERS['random_forest']
            self.clf = joblib.load(model_folder / 'subject_classifier.pkl')
            self.vectorizer = joblib.load(model_folder / 'vectorizer.pkl')
            self.label_encoder = joblib.load(model_folder / 'label_encoder.pkl')
            return True
        except FileNotFoundError:
            return False
    ...
    def get_model_files(self):
        model_folder = SUBJECT_MODEL_FOLDERS['random_forest']
        return [model_folder / 'subject_classifier.pkl', model_folder / 'vectorizer.pkl', model_folder / 'label_encoder.pkl']

class EmbeddingStrategy(ModelStrategy):
    """Embedding-based model strategy"""
    ...
    def load_model(self):
        try:
            model_folder = SUBJECT_MODEL_FOLDERS['embeddings']
            with open(model_folder / 'embedding_classifier.pkl', 'rb') as f:
    ...
    def predict(self, X_test_text):
        # This is simplified - you'd need to implement the embedding prediction logic
        # For now, return dummy predictions to demonstrate the framework
        return ["Ciencias de la computación e información"] * len(X_test_text)
    ...
    def get_model_files(self):
        model_folder = SUBJECT_MODEL_FOLDERS['embeddings']
        return [model_folder / 'embedding_classifier.pkl']

class MiniLMStrategy(ModelStrategy):
    """all-MiniLM-L6-v2 SVM model strategy"""
    ...
    def load_model(self):
        try:
            ...
            model_folder = SUBJECT_MODEL_FOLDERS['minilm']
            
            # Load SVM classifier
            self.classifier = joblib.load(model_folder / 'minilm_classifier.pkl')
            
            # Load label encoder
            self.label_encoder = joblib.load(model_folder / 'minilm_label_encoder.pkl')
            
            # Load model info
            with open(model_folder / 'minilm_model_info.pkl', 'rb') as f:
    ...
    def predict(self, X_test_text):
        if not all([self.classifier, self.label_encoder, self.transformer_model]):
            raise ValueError("Model not properly loaded")
        
        # Generate embeddings for test data
        test_embeddings = self.transformer_model.encode(
            X_test_text, 
            batch_size=32,
    ...
    def get_model_name(self):
        return "all-MiniLM-L6-v2 (SVM)"
    
    def get_model_files(self):
        model_folder = SUBJECT_MODEL_FOLDERS['minilm']
        return [
            model_folder / 'minilm_classifier.pkl',
            model_folder / 'minilm_label_encoder.pkl',
            model_folder / 'minilm_model_info.pkl'
        ]

class EmbeddingsKNNStrategy(ModelStrategy):
    """Embeddings + KNN model strategy"""
    ...
    def load_model(self):
        try:
            ...
            model_folder = SUBJECT_MODEL_FOLDERS['embeddings_knn']
            self.classifier = joblib.load(model_folder / 'embeddings_knn_classifier.pkl')
            self.label_encoder = joblib.load(model_folder / 'embeddings_knn_label_encoder.pkl')
            
            with open(model_folder / 'embeddings_knn_model_info.pkl', 'rb') as f:
                self.model_info = pickle.load(f)
            
            self.transformer_model = SentenceTransformer(self.model_info['embedding_model_name'])
            return True
        except (FileNotFoundError, ImportError) as e:
            print(f"Error loading Embeddings KNN model: {e}")
            return False
    
    def predict(self, X_test_text):
        test_embeddings = self.transformer_model.encode(X_test_text, batch_size=32, normalize_embeddings=True)
        y_pred_encoded = self.classifier.predict(test_embeddings)
        return self.label_encoder.inverse_transform(y_pred_encoded)
    ...
    def get_model_files(self):
        model_folder = SUBJECT_MODEL_FOLDERS['embeddings_knn']
        return [
            model_folder / 'embeddings_knn_classifier.pkl',
            model_folder / 'embeddings_knn_label_encoder.pkl',
            model_folder / 'embeddings_knn_model_info.pkl'
        ]

class NeuralTorchStrategy(ModelStrategy):
    """PyTorch Neural Network model strategy"""
    ...
    def load_model(self):
        ...
        try:
            ...
            model_folder = SUBJECT_MODEL_FOLDERS['neural']
            
            checkpoint = torch.load(model_folder / 'neural_torch_classifier.pth', map_location=self.device)
            ...
            self.label_encoder = joblib.load(model_folder / 'neural_torch_label_encoder.pkl')
    ...
    def predict(self, X_test_text):
        test_embeddings = self.transformer_model.encode(X_test_text, batch_size=32, normalize_embeddings=True)
        X_test_tensor = torch.FloatTensor(test_embeddings).to(self.device)
    ...
    def get_model_files(self):
        model_folder = SUBJECT_MODEL_FOLDERS['neural']
        return [
            model_folder / 'neural_torch_classifier.pth',
            model_folder / 'neural_torch_label_encoder.pkl'
        ]
...
class ModelComparator:
    ...
    def __init__(self):
        self.strategies = {
            'svm': SVMStrategy(kernel='linear'),
            'random_forest': RandomForestStrategy(),
            'xgboost': XGBoostStrategy(),
            'embeddings': EmbeddingStrategy(),
            'embeddings_knn': EmbeddingsKNNStrategy(),
            'neural': NeuralTorchStrategy(),
            'minilm': MiniLMStrategy()
        }
        self.test_data = None
        self.results = {}
    ...
    def print_comparison_table(self):
        ...
        if fastest_pred.model_name == best_accuracy.model_name:
            ...
            if abs(best_accuracy.f1 - fastest_pred.f1) < 0.02:  # Very close accuracy
                ...
                print(f"   ⚖️ Trade-off: {fastest_pred.model_name} for speed ({fast_pred_ms:.1f}ms) vs {best_accuracy.model_name} for accuracy (F1: {best_accuracy.f1:.3f})")
